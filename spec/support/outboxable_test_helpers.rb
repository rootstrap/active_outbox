module OutboxableTestHelpers
  ...
  matcher :create_transactional_outbox_record do
    match(notify_expectation_failures: true) do |actual|
      count = ActiveOutbox::Outbox.count
      expect { actual.call }.to change(ActiveOutbox::Outbox, :count).by_at_least(1)
      count = ActiveOutbox::Outbox.count - count

      if @attributes
        @attributes = @attributes.call if @attributes.is_a? Proc
        outboxes = ActiveOutbox::Outbox.last(count)
    ...
    match_when_negated(notify_expectation_failures: true) do |actual|
      expect { actual.call }.to_not change(ActiveOutbox::Outbox, :count)
    end
